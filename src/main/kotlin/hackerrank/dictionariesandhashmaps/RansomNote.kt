...
fun checkMagazine(magazine: Array<String>, note: Array<String>): Unit {
    ...
    for(i in 0 until magazine.size) {
        magazineMap.put(magazine[i], magazineMap.getOrDefault(magazine[i], 0) + 1)
    }
    ...
    for(entry in noteMap.entries) {
        if (magazineMap.getOrDefault(entry.key, 0) < entry.value) {
            print("No")
            return
        }
    }
...
/*fun checkMagazine(magazine: Array<String>, note: Array<String>): Unit {
    ...
    val magazineWordMap = magazine.distinct().associateWith { word -> magazine.count { it == word } }
    val noteWordMap = note.distinct().associateWith { word -> note.count { it == word } }

    for(entry in noteWordMap.entries) {
        if (magazineWordMap.getOrDefault(entry.key, 0) < entry.value) {
            print("No")
            return
        }
    }
